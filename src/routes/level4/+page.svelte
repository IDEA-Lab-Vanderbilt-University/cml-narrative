<script lang="ts">
	...
	import script from '$lib/scripts/level4old/index.js';
	...
	let hasTriedTraininatorInLevel4 = false;
	let hasTriedCodeinatorInLevel4 = false;
	let hasCompletedTrainAndCodeinatorInLevel4 = false;
	...
	$: hasTriedTraininatorInLevel4 = Boolean($studentProgressStore?.level4old_traininator_tried);
	$: hasTriedCodeinatorInLevel4 = Boolean($studentProgressStore?.level4old_codeinator_tried);
	$: hasCompletedTrainAndCodeinatorInLevel4 = hasTriedTraininatorInLevel4 && hasTriedCodeinatorInLevel4;

	$: if (lineNumber == 25 && !$studentProgressStore?.level4old_traininator_tried) {
		studentProgressStore.update((progress) => {
			progress.level4old_traininator_tried = true;
			return progress;
		});
	}
	...
	const handleNavigation = async (direction: NavigationDirection) => {
		...
		if (direction == NavigationDirection.forward) {
			if (line.id == script.lines.length) {
				...
				if (line.id === 13) {
					target = '/level4old?page=14&tutorialStep=1';
				} else if(line.id > 2 || line.id == 1) {
					target = `/level4old?page=${line.id + 1}`;
				}
			}
	...
	const getSurveyQuestionAudio = (index: number) => {
		...
		if ((surveyLanguage === 'en' || surveyLanguage === 'es') && basePath.startsWith('/survey/')) {
			return basePath.replace('/survey/', '/level4old/survey/');
		}
...
<Scene background={line.background} audio={line.audio}>
	...
	<div slot="content" class="h-full w-full" bind:this={content}>
        ...
        {#if lineNumber == 2}
            <TimeTravel destinationPage="/level4old?page=3" direction='backward' />
        {/if}
			...
			<Tablet showMeter={false} showBottomButtons={false}>
				<div class="agent-example">
					...
					<div class="agent-example-body">
						...
						{#if lineNumber == 8}
							<img src="/img/level4old/allfoods.png" alt="All Foods Example" class="agent-nova-image" />
						{/if}
						...
						<div class="flex flex-row">
							<div class="flex flex-col text-center">
								...
								<img src="/img/level4old/healthyfoods.png" alt="Healthy Foods Example" class="agent-nova-image agent-nova-image-highlight mr-4" />
							</div>
							<div class="flex flex-col text-center">
								<p>{$settingsStore?.language === 'es' ? 'Comidas no saludables' : 'Unhealthy Foods'}</p>
								<img src="/img/level4old/unhealthyfoods.png" alt="Unhealthy Foods Example" class="agent-nova-image agent-nova-image-highlight" />
							</div>
						</div>
						{/if}

						{#if lineNumber == 10}
							<img src="/img/level4old/objects.png" alt="Objects" class="agent-nova-image" />
						{/if}
						...
						<div class="flex flex-row">
							<div class="flex flex-col text-center">
								...
								<img src="/img/level4old/safeobjects.png" alt="Safe Objects Example" class="agent-nova-image agent-nova-image-highlight mr-4" />
							</div>
							<div class="flex flex-col text-center">
								<p>{$settingsStore?.language === 'es' ? 'Objetos afilados' : 'Sharp Objects'}</p>
								<img src="/img/level4old/dangerousobjects.png" alt="Dangerous Objects Example" class="agent-nova-image agent-nova-image-highlight" />
							</div>
						</div>
						{/if}

						{#if lineNumber == 12}
							<img src="/img/level4old/emotions.png" alt="Emotions Example" class="agent-nova-image" />
						{/if}
						...
						{#if lineNumber == 13}
							<div class="flex flex-row">
								<div class="flex flex-col text-center">
									...
									<img src="/img/level4old/positiveemotions.png" alt="Positive Emotions Example" class="agent-nova-image agent-nova-image-highlight mr-4" />
								</div>
								<div class="flex flex-col text-center">
									<p>{$settingsStore?.language === 'es' ? 'Emociones negativas' : 'Negative Emotions'}</p>
									<img src="/img/level4old/negativeemotions.png" alt="Negative Emotions Example" class="agent-nova-image agent-nova-image-highlight" />
								</div>
							</div>
						{/if}
				...
				{#if lineNumber == 14}
					...
					{#if tutorialStep >= 1 && tutorialStep <= 7}
						<div class="tutorial-dialog-banner tutorial-dialog-banner-full">
							...
							{/if}
							{#if tutorialStep >= 1 && tutorialStep <= 7}
								<AudioPlayer src={`/level4old/tutorial/step_${tutorialStep}.mp3`} />
							{/if}
						</div>
		...
		{#if lineNumber == 18}
			<Tablet showMeter={false} showBottomButtons={false}>
				<div class="robostepintro">
					...
					<p>{line.dialog()}</p>
					<button class="nextBtn page18-next" on:click={() => goto('/level4old?page=19')} disabled={!$audioPlaybackFinished}><img src="/img/misc/nextbutton.png" alt="Next" id="nextbutton" />
					</button>
				</div>
			</Tablet>
		{/if}
		{#if lineNumber == 19}
			<TextResponseModal 
				...
				onSuccess={(responses) => {					
					...
					goto('/level4old?page=22');
				}}
			/>
			<ChatbotWidget assistantId={chatbotAssistantId} historyScope="level4old-design-notes" preserveSpeechPunctuation={true} className="design-notes-chatbot" emptyMessageText={$settingsStore?.language === 'es' ? 'Di hola para comenzar' : 'say hello to get started'} />
		{/if}
		{#if lineNumber == 22}
			<Tablet showMeter={false} showBottomButtons={false}>
				<div class="robostepintro">
					...
					<button class="nextBtn" on:click={() => {
						studentProgressStore.update((progress) => {
							progress.last_visited = '/level4old?page=23';
							return progress;
						});
						
						goto('/level4old?page=23');
					}}>
				...
				<div class="train-test-hub">
					<div class="train-test-hub-top">
						<div class="train-test-hub-card">
							<button class="train-test-hub-icon-button" on:click={() => goto('/level4old?page=25')}>
						...
						<div class={`train-test-hub-card ${hasTriedTraininatorInLevel4 ? '' : 'train-test-hub-card-disabled'}`}>
							<button
								class="train-test-hub-icon-button"
								disabled={!hasTriedTraininatorInLevel4}
								on:click={() => goto('/level4old?page=26')}
							>
					...
					<div class="train-test-hub-notes">
						<button class="train-test-hub-icon-button" on:click={() => goto('/level4old?page=24')}>
					...
					<button class="nextBtn page23-done-next" disabled={!hasCompletedTrainAndCodeinatorInLevel4} on:click={() => {
						studentProgressStore.update((progress) => {
							progress.last_visited = '/level4old?page=27';
							return progress;
						});

						goto('/level4old?page=27');
					}}>
				...
				<TextResponseModal 
					...
					onSuccess={(responses) => {
						...
						const returnPageFromQuery = typeof window !== 'undefined'
							...
							: null;
						const targetPage = returnPageFromQuery === '35' ? 35 : 23;
						const targetUrl = targetPage === 35
							? '/level4old?page=35&returnPage=35&showTablet=1'
							: '/level4old?page=23';

						goto(targetUrl);
					}}
				/>
				<ChatbotWidget assistantId={chatbotAssistantId} historyScope="level4old-design-notes" className="design-notes-chatbot" preserveSpeechPunctuation={true} emptyMessageText={$settingsStore?.language === 'es' ? 'Di hola para comenzar' : 'say hello to get started'} />
			{/if}
		{/if}
		{#if lineNumber == 25}
			<Tablet showMeter={false} showBottomButtons={false}>
				...
				<Codinator 
					...
					on:submitted={() => {
						...
						const targetUrl = targetPage === 34
							? '/level4old?page=34&returnPage=34&showTablet=1'
							: targetPage === 35
								? '/level4old?page=35&returnPage=35&showTablet=1'
								: '/level4old?page=23';

						studentProgressStore.update((progress) => {
								progress.level4old_codeinator_tried = true;
							progress.last_visited = targetUrl;
							return progress;
						});
						goto(targetUrl);
