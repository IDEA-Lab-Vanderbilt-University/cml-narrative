<script lang="ts">
	...
	const startRecording = (itemId: string) => {
		...
		recognition.lang = $settingsStore?.language === 'es' ? 'es-US' : 'en-US';
				...
				<div class="response-column">
					...
					{#if item.singleLine}
						<input
							...
							placeholder={item.placeholder || ($settingsStore?.language === 'es' ? 'Escribe tu respuesta...' : 'Enter your response...')}
					...
					{#if canRecord}
						<button
							...
							{#if recordingItemId === item.id}
								...
								<span>{$settingsStore?.language === 'es' ? 'Detener grabación' : 'Stop Recording'}</span>
							{:else}
								<svg
									class="record-icon"
									xmlns="http://www.w3.org/2000/svg"
								...
								<span>{$settingsStore?.language === 'es' ? 'Comenzar a grabar' : 'Start Recording'}</span>
							{/if}
						</button>
					{/if}
				</div>
			{/each}
			<button class="submit-button" on:click={handleSubmit} disabled={!allResponsesFilled}>{submitButtonText}</button>
		</div>
	{:else}
		<TextResponse 
